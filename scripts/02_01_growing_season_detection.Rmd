...
#  i = 2
  ...
  if (sum(!is.na(a_DD$GPP_DT_VUT_REF)) >= 5*365 & !site_ID %in% c("BR-Sa1", "US-NC2", "US-SP3", "US-WCr", "US-Ho1", "US-Ho2", "US-NC4", "US-Kon", "US-MOz", "CH-Lae", "US-Oho")) {
  ...
  #
  ####moving growing season division
  if (site_ID %in% c("CA-Gro", "CL-SDF", "US-BZB", "US-BZS", "US-EML", "US-GLE", "US-Ho1", "US-Ho2", "US-Kon", "US-MOz", "US-NC4", "US-Oho", "US-Syv", "US-TP4", "US-los", "US-NC2", "US-PFa", "US-Uaf", "US-WCr", "BE-Bra", "BE-Dor", "BE-Maa", "CZ-BK1", "CZ-Lnz", "DE-HoH", "FI-Hyy", "FI-Sii", "FR-Bil", "FR-FBn", "FR-Hes", "IT-Ren", "SE-Deg", "SE-Svb", "CZ-RAJ", "CZ-Stn", "DE-Akm", "DE-Hai", "DE_Hzd", "ES_Agu", "ES_LJu", "FI-Let", "IT-Lav", "CH-Aws")) {
    brks <- season_mov(INPUT,
        list(FUN = "smooth_wWHIT", wFUN = wFUN,
            maxExtendMonth = 2,
            wmin = 0.0, r_min = 0.00,
...
gw_trend %>% left_join(sites_long, by="site_ID") %>% filter(category=='cold') %>%
  ggplot(aes(x=abs(Lat), y=eos_TRS)) +      # End of growing season does not change!
  geom_point() +
...
## Growing season declines; it starts later and end earlier for ENF (this is obvious!)
...
## Growing season does not change;
...
## increase in growing season length, it starts early/later and ends later. 

data <- gw_trend %>% left_join(sites_long, by="site_ID") %>% 
  filter(category=='cold' & IGBP %in% c('ENF', 'DBF', 'MF'))
summary(aov(data=data, length_gs ~ IGBP))  # p = 0.299

summary(aov(data=data, length_gs_piece ~ IGBP))  # p = 0.185

summary(aov(data=data, sos_DER ~ IGBP))  # p = 0.904

summary(aov(data=data, sos_TRS ~ IGBP))  # p = 0.61

summary(aov(data=data, eos_DER ~ IGBP))  # p = 0.134

summary(aov(data=data, eos_TRS ~ IGBP))  # p = 0.0916 .       marginal difference. 
