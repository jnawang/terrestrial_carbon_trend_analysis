...
#
library(librarian)
shelf(tidyverse, ggpubr, lme4, dagitty, ggridges, patchwork, cowplot, foreign, MASS, ggtext, trend, tidyterra, terra, ggExtra, ggforce, png, grid)
#
```
...
ggplot(data=carbon_sink) +
  ...
  labs(x='Year', y='Global annual terrestrial carbon sink (Pg C)', tag='a') +
  theme_classic()
...
for (i in 1:5) {
  if (i==1) {
    ...
    data <- sub_time_series %>% filter(category=='cold' & !IGBP %in% c('DBF', 'MF', 'ENF', 'EBF')) %>% filter(GPPtrend_lm < 100)
  }
...
#  geom_richtext(data=df_signif, aes(x=flux_type, y=y, label=label), fill=NA, label.color=NA, size=3) +
  ...
  #   y_position = 0.38, 
  #   color = "black") +  
  labs(x='Carbon flux types', y=expression("Trend (g C m"^{-2}~"year"^{-2}~")"), tag='a') + 
...
p2 <- data_plot %>%
  ...
  labs(x=expression("GPP trend (g C m"^{-2}~"year"^{-2}~")"), y=expression("NEE trend (g C m"^{-2}~"year"^{-2}~")"), tag='b') + 
...
data_plot3 <- data_plot %>% filter(group %in% c('2. All ecosystems in wet-warm climate', '3. ENF and EBF in wet-cold climate', '5. Non-forests in wet-cold climate')) %>% filter(GPPtrend_lm < 100)
...
data_plot4 <- data_plot %>% filter(!group %in% c('2. All ecosystems in wet-warm climate', '3. ENF and EBF in wet-cold climate', '5. Non-forests in wet-cold climate')) 
  ...
  df_segment[1, 3:4] <- intercept_uptake + slope_uptake * df_segment[1, 2]    # this can be modified
...
p3 <- ggplot(data=data_plot3, aes(x=GPPtrend_lm, y=ERtrend_lm)) + 
  ...
  labs(x=expression("GPP trend (g C m"^{-2}~"year"^{-2}~")"), y=expression("ER trend (g C m"^{-2}~"year"^{-2}~")"), tag='c') + 
  geom_text(x = -60, y = 78, label = paste(
    ...
    sep = "\n"), hjust = 0, size = 3.5) +
  geom_text(x = 60, y = -40, label = expression(atop(italic("r") ~ '= 0.58, ' ~ italic('p') ~ ' < 0.001', italic("slope") ~ " = " ~ bold("0.61") ~" ± 0.10")), size = 3) +
  theme_classic()
...
p4 <- ggplot(data=data_plot4, aes(x=GPPtrend_lm, y=ERtrend_lm)) + 
  ...
  scale_x_continuous(limits = c(-60, 90), breaks = seq(-60, 90, 30)) +
  scale_y_continuous(limits = c(-60, 90), breaks = seq(-60, 90, 30)) +
  labs(x=expression("GPP trend (g C m"^{-2}~"year"^{-2}~")"), y=expression("ER trend (g C m"^{-2}~"year"^{-2}~")"), tag='d') + 
...
ps1 <- data_plot %>% mutate(significance = case_when(group %in% c("2. All ecosystems in wet-warm climate") ~ "no", 
  ...
  labs(x=expression("ER trend (g C m"^{-2}~"year"^{-2}~")"), y=expression("NEE trend (g C m"^{-2}~"year"^{-2}~")"), tag='a') + 
...
ps3 <- data_plot %>% filter(group == '4. DBF and MF in wet-cold climate') %>%
  ...
  scale_shape_manual(values = c(1, 5)) +
  guides(shape = "none") +
  labs(x=expression("ER trend (g C m"^{-2}~"year"^{-2}~")"), y=expression("NEE trend (g C m"^{-2}~"year"^{-2}~")"), tag='c') +
  theme_bw() + 
...
#  geom_violin(scale="area") +
  geom_hline(yintercept=0, linetype = "dotted", colour = "grey") +
  labs(x='Before and after trend breakpoint year', y=expression("NEE trend (g C m"^{-2}~"year"^{-2}~")"), tag ='c') + 
...
p5 <- sub_time_series_2period_paired %>% mutate(period = factor(period, levels=c('Before', 'After'))) %>%
  ...
  geom_dotplot(binaxis = "y", stackdir = "center", alpha = 0.2) +
  geom_hline(yintercept=0, linetype = "dotted", colour = "grey") +
  labs(x='Before and after trend breakpoint year', y=expression("Carbon flux trend (g C m"^{-2}~"year"^{-2}~")"), tag ='d') +
...
ggsave(file.path(dir_fig, 'Fig3_trend_shift_long-term_sites.png'), width = 10, height = 8, units = "in")
