...
#
library(librarian)
shelf(tidyverse, Kendall, Rbeast, cowplot, ggpubr, tidyterra, ggpubr, viridis, RColorBrewer, grid, terra)
#
```
  ...
  if (site_ID %in% abrupt_change$site_ID) {
    ...
    for (iyear_ac in 1:length(years_abrupt_change)) {
      if (years_abrupt_change[iyear_ac] - year_start + 1 >= threshold_nyear - 1) {
        ...
        sub_time_series[isub_ts, c("NEEtrend_lm", "NEEp")] <- summary(lm(data=a_YY_sub, NEE_scale ~ year_center))$coefficients[2, c(1,4)]
        #
        # use daytime GPP and ER when it is available, otherwise use other time series
        # remove GPP and ER outliers; 
        if (sum(!is.na(a_YY_sub$GPP_DT_VUT_REF)) >= 5 & !site_ID %in% c("BR-Sa1", "US-NC2", "US-SP3", "US-WCr", "US-Ho1", "US-Ho2", "US-NC4", "US-Kon", "US-MOz", "CH-Lae", "US-Oho", "DE-RuW")) {
          ...
          sub_time_series[isub_ts, c("GPPtrend_lm", "GPPp")] <- summary(lm(data=a_YY_sub, scale(GPP_DT_VUT_REF) ~ year_center))$coefficients[2, c(1,4)]
          sub_time_series[isub_ts, "ER"] <- mean(a_YY_sub$RECO_DT_VUT_REF, na.rm=T)
          sub_time_series[isub_ts, c("ERtrend_lm", "ERp")] <- summary(lm(data=a_YY_sub, scale(RECO_DT_VUT_REF) ~ year_center))$coefficients[2, c(1,4)]
          #
          ...
          if (sum(!is.na(a_YY_sub$GPP_NT_VUT_REF)) >= 5) {
            ...
            sub_time_series[isub_ts, c("GPPtrend_lm", "GPPp")] <- summary(lm(data=a_YY_sub, scale(GPP_NT_VUT_REF) ~ year_center))$coefficients[2, c(1,4)]
            sub_time_series[isub_ts, "ER"] <- mean(a_YY_sub$RECO_NT_VUT_REF, na.rm=T)
            sub_time_series[isub_ts, c("ERtrend_lm", "ERp")] <- summary(lm(data=a_YY_sub, scale(RECO_NT_VUT_REF) ~ year_center))$coefficients[2, c(1,4)]
            #
            if (abs(sub_time_series[isub_ts, "GPPtrend_lm"]) > 0.25 | abs(sub_time_series[isub_ts, "ERtrend_lm"]) > 0.25) {
