...
#
library(librarian)
shelf(tidyverse, ggpubr, MuMIn, car, fBasics, olsrr, lme4, Hmisc, corrplot, trend)
#
```
...
data_ll <- suppressWarnings(terra::project(TNdep2008_2020, "EPSG:4326"))
...
#
sub_time_series <- read.csv('data/sub_time_series_sen.csv')
#
...
```{r a function to calculate sens slope}
...
#
rm_years_co2 <- read.csv('data/bad_co2_site_year.csv')
#
...
for (i in 1:nrow(sub_time_series)) {
  ...
  if (! 'CO2_F_MDS' %in% names(a_YY) ) {
    ...
    if (site_ID %in% rm_years_co2$site_ID) {
      rm_years_co2_1site = rm_years_co2[rm_years_co2$site_ID==site_ID, 2:ncol(rm_years_co2)]
      rm_years_co2_1site = rm_years_co2_1site[!is.na(rm_years_co2_1site)]
      a_YY$CO2_F_MDS[a_YY$TIMESTAMP %in% rm_years_co2_1site] <- NA
    }
    if ( sum(!is.na(a_YY$CO2_F_MDS)) < 5 ) {
      print(paste0('not enought CO2 data-', site_ID))
    }
  }
  # # temporally output the CO2 time series
  if (!all(is.na(a_YY$CO2_F_MDS))) {
  ...
  if (site_ID %in% c('BR-Sa1', 'US-Esm', "US-Elm", 'US-LL1', 'US-LL2', 'US-LL3', 'US-SP3', 'GF-Guy')) {
    ...
    # deal with missed P_ERA data in some years
    missed_PERA_years <- a_YY$TIMESTAMP[is.na(a_YY$P_ERA)]
    a_YY_gw$P_ERA[a_YY_gw$year %in% missed_PERA_years] <- NA
    a_YY_ngw$P_ERA[a_YY_ngw$year %in% missed_PERA_years] <- NA
    #
  ...
  #
  # Calculate trends
  sub_time_series[i, c("TA_t", "TA_p")] <- cal_sens_slope(a_YY$TA_F_MDS)
    # summary(lm(data=a_YY, scale(TA_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("TA_gs_t", "TA_gs_p")] <- cal_sens_slope(a_YY_gw$TA_F_MDS)
    # summary(lm(data=a_YY_gw, scale(TA_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("TA_ngs_t", "TA_ngs_p")] <- cal_sens_slope(a_YY_ngw$TA_F_MDS)
    # summary(lm(data=a_YY_ngw, scale(TA_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  #
  sub_time_series[i, c("P_t", "P_p")] <- cal_sens_slope(a_YY$P_ERA)
    # summary(lm(data=a_YY, scale(P_ERA) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("P_gs_t", "P_gs_p")] <- cal_sens_slope(a_YY_gw$P_ERA)
    # summary(lm(data=a_YY_gw, scale(P_ERA) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("P_ngs_t", "P_ngs_p")] <- cal_sens_slope(a_YY_ngw$P_ERA)
    # summary(lm(data=a_YY_ngw, scale(P_ERA) ~ year_center))$coefficients[2, c(1,4)]
  #
  sub_time_series[i, c("VPD_t", "VPD_p")] <- cal_sens_slope(a_YY$VPD_F_MDS)
    # summary(lm(data=a_YY, scale(VPD_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("VPD_gs_t", "VPD_gs_p")] <- cal_sens_slope(a_YY_gw$VPD_F_MDS)
    # summary(lm(data=a_YY_gw, scale(VPD_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("VPD_ngs_t", "VPD_ngs_p")] <- cal_sens_slope(a_YY_ngw$VPD_F_MDS)
    # summary(lm(data=a_YY_ngw, scale(VPD_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  #
  sub_time_series[i, c("SW_t", "SW_p")] <- cal_sens_slope(a_YY$SW_IN_F_MDS)
    # summary(lm(data=a_YY, scale(SW_IN_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("SW_gs_t", "SW_gs_p")] <- cal_sens_slope(a_YY_gw$SW_IN_F_MDS)
    # summary(lm(data=a_YY_gw, scale(SW_IN_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("SW_ngs_t", "SW_ngs_p")] <- cal_sens_slope(a_YY_ngw$SW_IN_F_MDS)
    # summary(lm(data=a_YY_ngw, scale(SW_IN_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  #
  sub_time_series[i, c("LE_t", "LE_p")] <- cal_sens_slope(a_YY$LE_F_MDS)
    # summary(lm(data=a_YY, scale(LE_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("LE_gs_t", "LE_gs_p")] <- cal_sens_slope(a_YY_gw$LE_F_MDS)
    # summary(lm(data=a_YY_gw, scale(LE_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  sub_time_series[i, c("LE_ngs_t", "LE_ngs_p")] <- cal_sens_slope(a_YY_ngw$LE_F_MDS)
    # summary(lm(data=a_YY_ngw, scale(LE_F_MDS) ~ year_center))$coefficients[2, c(1,4)]
  #
  # left join a_YY, a_YY_gw, and a_YY_ngw and put data into a data frame
  a_YY_gw_rename <- a_YY_gw
  a_YY_ngw_rename <- a_YY_ngw
  if ("year" %in% names(a_YY_gw_rename)) {
    a_YY_gw_rename <- a_YY_gw_rename %>% rename('TIMESTAMP' = "year")
  }
  if ("year" %in% names(a_YY_ngw_rename)) {
    a_YY_ngw_rename <- a_YY_ngw_rename %>% rename('TIMESTAMP' = "year")
  }
  #
  varnames_select <- c("TIMESTAMP", "TA_F_MDS", "P_ERA", "VPD_F_MDS", "SW_IN_F_MDS", "LE_F_MDS")
  a_YY_gw_rename <- a_YY_gw_rename[, varnames_select]
  names(a_YY_gw_rename)[2:6] <- paste0(varnames_select[2:6], "_gw")
  a_YY_ngw_rename <- a_YY_ngw_rename[, varnames_select]
  names(a_YY_ngw_rename)[2:6] <- paste0(varnames_select[2:6], "_ngw")
  #
  merged_df <- merge(a_YY[, c("NEE_VUT_REF", varnames_select)], a_YY_gw_rename, by="TIMESTAMP")
  merged_df <- merge(merged_df, a_YY_ngw_rename, by="TIMESTAMP")
  merged_df$site_ID <- site_ID
  merged_df$isubts <- i
  data_trend_driver <- rbind(data_trend_driver, merged_df)
  #
  ## we do not differentiate CO2, Lgs for growing and non-growing seasons
  if (sum(!is.na(a_YY$CO2_F_MDS)) > 2) {
#    sub_time_series[i, c("CO2_t", "CO2_p")] <- cal_sens_slope(a_YY$CO2_F_MDS)
    # I should not standarize CO2, because I want to know how per ppm CO2 increase affect NEE trend. 
    sub_time_series[i, c("CO2_t", "CO2_p")] <- summary(lm(data=a_YY, CO2_F_MDS ~ year_center))$coefficients[2, c(1,4)]
    # deal with abnormal data; the standard deviation is 0.9 ppm/year; mean = 2.36 ppm/year
    if (sub_time_series$CO2_t[i] > 4.3 | sub_time_series$CO2_t[i] < 0.5) {
      sub_time_series[i, c("CO2_t", "CO2_p")] <- NA
    }
  }
  if (site_ID %in% c('BR-Sa1', 'US-Esm', "US-Elm", 'US-LL1', 'US-LL2', 'US-LL3', 'US-SP3', 'GF-Guy')) {
    ...
    if (nrow(subdata_grow_season) >= 5) {
      sub_time_series[i, c("Lgs_t", "Lgs_p")] <- cal_sens_slope(subdata_grow_season$length_gs)
        # summary(lm(data=subdata_grow_season, scale(length_gs) ~ scale(year, scale=FALSE)))$coefficients[2, c(1,4)]
      sub_time_series[i, c("Lgs_piece_t", "Lgs_piece_p")] <- cal_sens_slope(subdata_grow_season$length_gs_piece)
        # summary(lm(data=subdata_grow_season, scale(length_gs_piece) ~ scale(year, scale=FALSE)))$coefficients[2, c(1,4)]
    }
...
write.csv(sub_time_series, "data/sub_time_series_potential_drivers_sen.csv", row.names = F)

write.csv(data_trend_driver, "data/data_trend_driver_sen.csv", row.names = F)

```


```{r any relationships between CO2 and NEE trend}
...
sub_time_series %>% 
  ...
  ggplot(aes(x=CO2_t, y=NEEtrend_lm)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()

sub_time_series %>% 
  filter(IGBP %in% c('DBF', 'MF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=NEEtrend_lm, color=IGBP)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()

sub_time_series %>% 
  filter(!IGBP %in% c('ENF', 'EBF', 'DBF', 'MF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=NEEtrend_lm)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()
...
###I may detect fertilization effect only if I have many sites for each climate. it is very weak in this data. 

###How about correlation with GPP; The relationship is converse;
sub_time_series %>% 
  filter(IGBP %in% c('ENF', 'EBF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=GPPtrend_lm)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()
...
### but climate's effect is there; local CO2 concentration has effects on TA and SW trend
sub_time_series %>% 
  filter(IGBP %in% c('ENF', 'EBF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=TA_gs_t)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()
...
sub_time_series %>% 
  filter(IGBP %in% c('DBF', 'MF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=TA_gs_t, col=IGBP)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor()

# there is global spatial heterogeneity there; why they are different?
sub_time_series %>% 
  filter(!IGBP %in% c('ENF', 'EBF', 'DBF', 'MF') & category == "cold") %>% 
  ggplot(aes(x=CO2_t, y=TA_gs_t, col=IGBP)) +
  geom_point() +
...
# this is related to vegetation types! no effects. 
sub_time_series %>% 
  filter(category == "dry") %>% 
  ggplot(aes(x=CO2_t, y=TA_gs_t, col=IGBP)) +
  geom_point() +
...
model_best <- lm(NEEtrend_lm ~ LE_t + SW_t + IGBP, data = data_wet)
summary(model_best)
...
data_use %>% 
  ggplot(aes(x=P_gs_t, y=NEEtrend_lm)) + 
  geom_point() +
  geom_smooth(method = lm) +
  stat_cor(color='green') 
...
data_DBF_cold %>% 
  ggplot(aes(x=LE_gs_t, y=NEEtrend_lm)) + 
  geom_point() +
  geom_smooth(method = lm) +
  stat_cor(color='green') 
