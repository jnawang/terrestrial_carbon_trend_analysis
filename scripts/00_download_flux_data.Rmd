...
dir.rawdata <- '/Users/jw2946/Documents/stability_project/SiteData/'
...
```{r download datasets}
packages <- c("ggplot2", "dplyr", "tidyr", "amerifluxr", "data.table", "pander", "lubridate")
#
for (pkg in packages) {
...
```{r European and American sites I want to use}
...
## US-Me6: this is a burned sites. Should I remove some disturbance years?
## US-Myb: restored wetland. 
#
sites.long <- sites.long %>% filter(!Site.ID %in% c('CA-TP1', 'US-Rpf', 'US-Me6', 'US-Myb'))
...
# I need to add roots into these files
files.unzip.Ameri <- list.files(file.path(dir.rawdata, "Ameriflux_FLUXNET", "unzip"), full.names = T)
files.unzip.Europ <- list.files(file.path(dir.rawdata, "FLUXNET2020", "unzip"), full.names = T)
#
sites.long <- sites.selected.FLUXNET
sites.long$Source <- "Ameri"
#
processed.site <- read.csv('/Users/jw2946/Documents/stability_project/code_trend_analysis_old/long_term_ec_sites.csv')
sites.long <- sites.long %>% filter(Site.ID %in% processed.site$Site.ID)
#
rm_years <- data.frame(Site.ID=character(), years=list())
#
for (i in 1:nrow(sites.long)) {
#  i = 27
  Site.ID <- sites.long$Site.ID[i]
  print(Site.ID)
  # AmeriFlux and European flux; Do I need to differentiate them? Hopefully they use the same notation
  if (sites.long$Source[i] == "Ameri") {
    if (Site.ID %in% c("US-Cop")) {
      a_HH <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_HR"), files.unzip.Ameri)])
      a_DD <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_DD"), files.unzip.Ameri)])
      a_YY <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_YY"), files.unzip.Ameri)])
    } else {
      a_HH <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_HH"), files.unzip.Ameri)])
      a_DD <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_DD"), files.unzip.Ameri)])
      a_YY <- read.csv(files.unzip.Ameri[grepl(pattern=paste0(Site.ID, "_FLUXNET_FULLSET_YY"), files.unzip.Ameri)]) 
    }
  } else {
    a_HH <- read.csv(files.unzip.Europ[grepl(pattern=paste0(Site.ID, "_FLUXNET2015_FULLSET_HH"), files.unzip.Europ)])
    a_DD <- read.csv(files.unzip.Europ[grepl(pattern=paste0(Site.ID, "_FLUXNET2015_FULLSET_DD"), files.unzip.Europ)])
    a_YY <- read.csv(files.unzip.Europ[grepl(pattern=paste0(Site.ID, "_FLUXNET2015_FULLSET_YY"), files.unzip.Europ)])
  }
  a_HH[a_HH==-9999] <- NA
  a_DD[a_DD==-9999] <- NA
  a_YY[a_YY==-9999] <- NA
  ...
  #
  plot(a_DD$TIMESTAMP, a_DD$GPP_DT_VUT_REF)
  plot(a_DD$TIMESTAMP, a_DD$GPP_DT_VUT_REF)
  #
  plot(a_HH$TIMESTAMP, a_HH$NEE_VUT_REF)
  # 
  #
  plot(ymd_hm(a_HH$TIMESTAMP_START[a_HH$NEE_VUT_REF_QC==0]), a_HH$NEE_CUT_REF[a_HH$NEE_VUT_REF_QC==0])
  #
  ...
  f.data.measured <- a_HH %>% group_by(year(TIMESTAMP)) %>% summarise(f=sum(NEE_VUT_REF_QC==0) / n(), name=Site.ID)
  ...
  #
  if (length(tmp) > 0) {
    #### remove these < 0.1, and then < 40% of the mean of these points;
    rm_years <- rbind(rm_years, data.frame(Site.ID=Site.ID, years=tmp))
    ####
  }
  #
}

# remove extra years due to mannual check
rm_years <- rbind(rm_years, data.frame(Site.ID="CA-Gro", years=c(2014)))
rm_years <- rbind(rm_years, data.frame(Site.ID="US-Rwf", years=c(2014)))
#
# these years cannot pass further check
...
rm_years <- rbind(rm_years, data.frame(Site.ID="US-Var", years=c(2021)))   # 2021
...
# save the sites with >= 7 years data. 

```

## Selected qualifed FLUXNET sites
```{r Ameriflux Fluxnet data quality check, echo=FALSE}
...
# generate sites and years with reliable data



# the sites we want to remove; the year we want to remove; 



## What gap-filled data are required? What are the standard variable names for this dataset? What are standard fill name format?
# AMF_CA-Ca1_FLUXNET_FULLSET_DD_1996-2010_EddyProc.csv
# AMF_CA-Ca1_FLUXNET_FULLSET_HH_1996-2010_EddyProc.csv
# AMF_CA-Ca1_FLUXNET_FULLSET_YY_1996-2010_EddyProc.csv


# a_HH$TIMESTAMP_START, a_HH$TIMESTAMP_END, a_HH$NEE_VUT_REF, a_HH$NEE_VUT_REF_QC [qualification flag, this is important], daytime, GPP_NT_VUT_REF, RECO_NT_VUT_REF, GPP_DT_VUT_REF, RECO_DT_VUT_REF
# how about variables related LE? CO2 concentration, meterological variables, TA_F_MDS, RH, WS_F, SW_IN_F_MDS, SWC
...
# a_YY$TIMESTAMP, -a_YY$NEE_VUT_REF, a_YY$GPP_DT_VUT_REF, a_YY$GPP_NT_VUT_REF, a_YY$RECO_DT_VUT_REF, a_YY$RECO_NT_VUT_REF, a_YY$TA_F_MDS, a_YY$VPD_F_MDS, a_YY$P_F
