...
#
# this following meta file is downloaded from Ameriflux with the search criteria: FC=true, NEE=true, data length >= 5 years for the file AmeriFlux-site-search-results-202501071742
# sites <- read.csv('data/AmeriFlux-site-search-results-202501071742.csv')
# for this file "AmeriFlux-site-search-results-202512092232.csv": we used FC=true, length >=10 years
...
sites.selected.FLUXNET <- sites.selected %>% filter(AmeriFlux.FLUXNET.Data.End - AmeriFlux.FLUXNET.Data.Start >= 10)
## 58 sites

# I have 55 sites to download, gap fill and partition
sites.selected.BASE <- sites.selected$Site.ID[(!sites.selected$Site.ID %in% sites.selected.FLUXNET$Site.ID)]
# base data was accessed in 12/10/2025
for (site in sites.selected.BASE) {
  ...
  verbose = TRUE,
  out_dir = file.path(dir_rawdata, 'Ameriflux_BASE')
)
}
...
files <- list.files(file.path(dir_rawdata, "Ameriflux_FLUXNET"), pattern = "\\.zip$", full.names = T)
exdir <- file.path(dir_rawdata, "Ameriflux_FLUXNET", "unzip")
# 
...
rm_years <- data.frame(Site.ID=character(), years=integer())
#
for (i in 112:nrow(sites_long)) {
  # i = 103
  ...
  if (sites_long$source[i] == "AmeriFlux_FLUXNET") {
    if (site_ID %in% c("US-Cop", "US-Ha1", "US-MMS")) {
...
#  par(mar=c(3, 3, 3, 3))
  ...
  #
  if (length(tmp) > 0) {
    rm_years <- rbind(rm_years, data.frame(Site.ID=site_ID, years=tmp))
  }
}
#
# write.csv(rm_years, file = 'data/remove_years_Ameriflux2025.csv', row.names = F)
```
