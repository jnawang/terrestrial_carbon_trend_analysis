...
data_length_thresh <- 10
#
files.unzip.Ameri <- list.files(file.path(dir_rawdata, "Ameriflux_FLUXNET", "unzip"), pattern =  "_FLUXNET_FULLSET_(HR|HH|DD|YY)", full.names = T)
files.unzip.FLUXNET2020 <- list.files(file.path(dir_rawdata, "FLUXNET2020", "unzip"), pattern = "_FLUXNET2015_FULLSET_(HR|HH|DD|YY)", full.names = T)
files.unzip.FLUXNET2015 <- list.files(file.path(dir_rawdata, "FLUXNET2015_10yr_only"), pattern = "", full.names = T)
...
for (i in 1:nrow(sites_long)) {
  ...
  if (!site_ID %in% c("BR-Sa1", "BR-Ma2", "GF-Guy", "ZM-Mon")) {
  ...
  if (!site_ID %in% c("BR-Sa1", "BR-Ma2", "ZM-Mon", "US-LL3", "US-LL1", "US-LL2", "GF-Guy", "CL-SDF", "US-SP1", "US-SP3")) {
...
#  if (i %in% c(15, 16, 35)) {next}  # 16, 35, 
  ...
  a_HH <- read.csv(folder[grepl(pattern=paste0(site_ID, HHfile), folder)])
  a_YY <- read.csv(folder[grepl(pattern=paste0(site_ID, YYfile), folder)])
  # get wind spead, RH, and solar radiation in the Ameriflux base data
  if (sites_long$source[i] == "AmeriFlux_BASE") {
