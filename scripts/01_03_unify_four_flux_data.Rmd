...
sites_long <- sites_long %>% filter(!site_ID %in% c('CA-TP1', 'US-Rpf', 'US-Me6', 'US-Myb', "CA-Ca2", "US-KLS", "US-NC1", "US-NC3", "US-Tw4", "US-SP2", "DE-Hzd"))
##
```
...
####
# I should use hourly pattern, or daily data to calculate PET
####
#
...
#
water_flux <- read.csv('data/water_flux_AI.csv')
#
sites_long <- sites_long %>% left_join(water_flux, by='site_ID')
#
sites_long <- sites_long %>% 
  mutate(category=case_when(Climate.class %in% c("Csa", "Csb", "Bsh", "Bsk", "BWh") & AI < 0.5 ~ 'dry',
                            Climate.class %in% c("Csa", "Csb", "Bsh", "Bsk", "BWh") & AI >= 0.5 ~ 'wet', 
...
# CL-SDF should be in wet climate, because it is tropical forests.         
sites_long$category[sites_long$site_ID=='CL-SDF'] <- "wet"
#
...
#
# we have 13 dry sites. 
# we have 17 wet sites.
# we have 73 cold sites. 
