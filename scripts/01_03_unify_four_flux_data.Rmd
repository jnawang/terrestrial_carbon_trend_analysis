...
sites_long <- sites_long %>% filter(!site_ID %in% c('CA-TP1', 'US-Rpf', 'US-Me6', 'US-Myb', "CA-Ca2", "US-KLS", "US-NC1", "US-NC3", "US-Tw4", "US-SP2", "CL-SDP"))
##
...
```{r remove some years}
##this chunk generates the years we should remove; it continues with the also check
#
...
#  i=7
  ...
  f.data.measured <- a_HH %>% group_by(year(TIMESTAMP)) %>% summarise(f=sum(NEE_VUT_REF_QC==0) / n())
#  print(f.data.measured)
  #
  tmp <- f.data.measured %>% filter(f > 0.1 & !is.na(f))
  meanf <- mean(tmp$f)
  #
  tmp <- f.data.measured$`year(TIMESTAMP)`[is.na(f.data.measured$f) | f.data.measured$f < 0.1]
  print(tmp)
  #
  #### The second rule to remove years: there are NA values in yearly data. 
  tmp <- c(tmp, a_YY$TIMESTAMP[is.na(a_YY$NEE_VUT_REF)])
  #### The third rule to remove years with extreme outliers: a year's NEE outside of 3 * sd, and data is less than the average. 
#  outlier <- abs(a_YY$NEE_VUT_REF - mean(a_YY$NEE_VUT_REF, na.rm=T)) > 3 * sd(a_YY$NEE_VUT_REF, na.rm=T)
  # detect outliers: Using the Interquartile Range (IQR) Method
  Q1 <- quantile(a_YY$NEE_VUT_REF, 0.25, na.rm=T)
  Q3 <- quantile(a_YY$NEE_VUT_REF, 0.75, na.rm=T)
  IQR <- Q3 - Q1
  outlier <- a_YY$NEE_VUT_REF < Q1 - 1.5 * IQR | a_YY$NEE_VUT_REF > Q3 + 1.5 * IQR
  tmp <- c(tmp, a_YY$TIMESTAMP[which(outlier & f.data.measured$f < meanf * 0.5)])
  #
  outlier_extreme <- a_YY$NEE_VUT_REF < Q1 - 3 * IQR | a_YY$NEE_VUT_REF > Q3 + 3 * IQR
  ...
  if (!is.na(outlier[1]) & outlier[1]) {
    tmp <- c(tmp, a_YY$TIMESTAMP[1])
  }
  #### the fifth rule: monthly average NEE during daytime between 11 and 14 should be within a bound ###
  # monthly_NEE_day <- a_HH %>% filter(NIGHT==0 & SW_IN_F_MDS > 10 & hour(TIMESTAMP) >= 11 & hour(TIMESTAMP) <= 13) %>%
  #   mutate(YEAR=year(TIMESTAMP), MONTH=month(TIMESTAMP)) %>% group_by(YEAR, MONTH) %>%
  #   summarise(NEE=mean(NEE_VUT_REF, na.rm=T))
  # monthly_NEE_day_pattern <- monthly_NEE_day %>% group_by(MONTH) %>%
  #   summarise(Q1=quantile(NEE,0.25, na.rm=T), Q3=quantile(NEE,0.75, na.rm=T)) %>%
  #   mutate(lowlimit=Q1 - 3.5 * (Q3 - Q1), uplimit=Q3 + 3.5 * (Q3 - Q1))
  # #
  # monthly_NEE_day <- monthly_NEE_day %>% left_join(monthly_NEE_day_pattern[,c(1,4:5)], by="MONTH") %>% filter(NEE > uplimit) %>% group_by(YEAR) %>% summarise(n=n()) %>% filter(n>=2)
  # #### a year has >= 3 month outliers
  # if (nrow(monthly_NEE_day) > 0) {
  #   tmp <- c(tmp, monthly_NEE_day$YEAR)
  #   print(paste0('remove monthly outliters: ', site_ID, monthly_NEE_day$YEAR))
  # }
  #
  ...
  if (!site_ID %in% c("BR-Sa1", "US-NC2", "US-SP3", "US-WCr", "US-Ho1", "US-Ho2", "US-NC4", "CA-Man", "US-LL1", "US-LL2", "US-PFa", "US-Uaf")) {
...
write.csv(rm_years, file = '../data/years_to_remove_NEE.csv', row.names = F)
##
