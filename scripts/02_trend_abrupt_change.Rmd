...
#
library(librarian)
shelf(tidyverse, Kendall, Rbeast, cowplot, ggpubr)
#
```
...
a_YY_all %>% group_by(TIMESTAMP) %>% summarise(nyears=n())
#
# the trend using moving windows. 
...
#
# if 
abrupt_change$cpAbruptChange[abrupt_change$site_ID=="US-SFa"] <- 2007
#
  ...
  if (site_ID %in% abrupt_change$site_ID) {
    ...
    for (iyear_ac in 1:length(years_abrupt_change)) {
      if (years_abrupt_change[iyear_ac] - year_start + 1 >= threshold_nyear - 1) {
        ...
        if (sum(!is.na(a_YY_sub$GPP_DT_VUT_REF)) >= 5 & !site_ID %in% c("BR-Sa1", "US-NC2", "US-SP3", "US-WCr", "US-Ho2", "US-NC4")) {
...
```{r analyzing trend results, fig.width=10, fig.height=4}
#
...
```{r analyzing trends in two different periods, fig.width=5, fig.height=5}
#
...
for (i in 1:nrow(sub_time_series)) {
  # ignore one sub time series at US-PFa
  if (sub_time_series$site_ID[i]=="US-PFa" & sub_time_series$year_start[i]==2008) {
    next
  }
  #
...
#
ggplot(sub_time_series_2period, aes(x=period, y=trend_lm)) +
  geom_jitter() +
  geom_boxplot()
#
sub_time_series_2period_paired <- sub_time_series_2period %>% filter(site_ID %in% unique(abrupt_change$site_ID))
sub_time_series_2period_paired_pivot <- sub_time_series_2period_paired %>% dplyr::select(c(-year_start, -year_end)) %>% pivot_wider(names_from = period, values_from = trend_lm) %>% filter(!is.na(before_2011) & !is.na(after_2011)) %>% left_join(sites_long[, c("site_ID", "IGBP", "Climate.class")], by="site_ID")
###
### removing drylands, NEE trend increased a lot (carbon sequestration reduced). 
sub_time_series_2period_paired_pivot <- sub_time_series_2period_paired_pivot %>% filter(Climate.class != "Csa")
t.test(sub_time_series_2period_paired_pivot$before_2011, sub_time_series_2period_paired_pivot$after_2011, paired=T)
#
sub_time_series_2period_paired %>% ggplot(aes(x=period, y=trend_lm)) +
  geom_jitter() +
  geom_boxplot()
#
...
#
data_NEE_trend$period[is.na(data_NEE_trend$period)] <- "cross_2011"
#
...
ggplot(data=trend_random, aes(x=trend_lm, y=trend_inla)) + 
  geom_point()
