...
#  i = 6
  ...
  data_EddyProc$NEE[abs(data_EddyProc$NEE) > 20] <- NA
  #
  ...
  if (name_site %in% c("CL-SDP", "US-Snd", "US-SP3")) {
    uStarTh <- EProc$sEstUstarThold(seasonFactor = usCreateSeasonFactorMonth(data_EddyProc$DateTime, month = month(data_EddyProc$DateTime), 
                                    ...
                                    ctrlUstarSub =usControlUstarSubsetting(taClasses = 7, UstarClasses = 20,
                                                                           ...
                                                                           isUsingOneBigSeasonOnFewRecords = TRUE))    # This works for no
  } else {
  ...
  a_HH <- a_HH %>% mutate(TA_F=output$Tair_f, TA_F_QC=output$Tair_fqc, VPD_F=output$VPD_f, VPD_F_QC=output$VPD_fqc, 
  ...
  a_DD <- a_HH %>% mutate(YEAR=year(ymd_hm(TIMESTAMP_START)), MONTH=month(ymd_hm(TIMESTAMP_START)),     
                   ...
                   group_by(TIMESTAMP) %>% summarise(TA_F=mean(TA_F), VPD_F=mean(VPD_F), LE_F_MDS=mean(LE_F_MDS), 
  ...
  a_YY <- a_HH %>% mutate(TIMESTAMP=year(ymd_hm(TIMESTAMP_START))) %>% group_by(TIMESTAMP) %>% 
                   summarise(TA_F=mean(TA_F), VPD_F=mean(VPD_F), LE_F_MDS=mean(LE_F_MDS), 
                             NEE_VUT_REF=sum(NEE_VUT_REF)*dt*0.0864*12)
  if (col_names_used[10]) {
...
a_HH <- data_BASE %>% select(c(TIMESTAMP_START, TIMESTAMP_END)) %>% 
  mutate(TA_F=data_BASE$TA_PI_F_1_3_1, TA_F_QC=ifelse(is.na(data_BASE$TA_1_3_1), 1, 0), 
         VPD_F=output$VPD_f, VPD_F_QC=output$VPD_fqc, 
...
a_DD <- a_HH %>% mutate(YEAR=year(ymd_hm(TIMESTAMP_START)), MONTH=month(ymd_hm(TIMESTAMP_START)),     
                        DAY=day(ymd_hm(TIMESTAMP_START)), TIMESTAMP=sprintf("%04d%02d%02d", YEAR, MONTH, DAY)) %>% 
                 group_by(TIMESTAMP) %>% summarise(TA_F=mean(TA_F), VPD_F=mean(VPD_F), 
...
#
a_YY <- a_HH %>% mutate(TIMESTAMP=year(ymd_hm(TIMESTAMP_START))) %>% group_by(TIMESTAMP) %>% 
                   summarise(TA_F=mean(TA_F), VPD_F=mean(VPD_F), LE_F_MDS=mean(LE_F_MDS), CO2_F_MDS=mean(CO2_F_MDS), 
