...
data_plot3 %>% filter(GPPtrend_lm <= -29) %>% 
  mutate(x_adj = GPPtrend_lm - -29, y_adj = ERtrend_lm - -19) %>%
  lm(data=., y_adj ~ x_adj + 0) %>% summary()
#       Estimate Std. Error t value Pr(>|t|)
# x_adj  -0.6113     0.7173  -0.852    0.427
...
data_plot4 <- data_plot %>% filter(!group %in% c('2. All ecosystems in wet-warm climate', '3. ENF and EBF in wet-cold climate', '5. Non-forests in wet-cold climate')) 
  ...
  df_segment[1, 3:4] <- -19.1 + -0.6113 * (df_segment[1, 1:2] - -29)   # this can be modified
