...
data_cold_nonforest <- data %>% filter(category == "cold") %>%
  ...
  dplyr::select(-c("category", "P_t", "SW_t", "LE_t", "Age")) # %>% filter(site_ID !='BE-Maa')
...
summary(lm(NEE_trend ~ Height + P_gs_t + SW_gs_t, data=data_cold_nonforest))  # R2 = 0.5645 / 0.5023  # + IGBP_new
