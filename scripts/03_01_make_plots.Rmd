...
#  geom_line() +
  labs(x='Year', y='Number of study sites') +
  theme_bw() +
...
#  rlmod <- rlm(data=data, NEE_trend ~ GPPtrend_lm + 0, psi = psi.bisquare)    # 
  ...
  # df_NEE_GPP_relation[i, 4] <- coefs[1,2]
  # df_NEE_GPP_relation[i, 5] <- pvals[1]
...
t.test(sub_time_series$NEE_trend, mu=0)    # t = 0.029238, df = 123, p-value = 0.9767;   mean of x 0.0003806223
sd(sub_time_series$NEE_trend)              # sd 0.15
range(sub_time_series$NEE_trend)           # -0.2859061  0.3658240

t.test(sub_time_series$GPPtrend_lm, mu=0)  # t = 2.8126, df = 123, p-value = 0.005722; mean of x 0.03489742

t.test(sub_time_series$ERtrend_lm, mu=0)   # t = 1.5873, df = 123, p-value = 0.115; mean = 0.01960323
...
#  mutate(label = paste0("y = <b>", round(slope, 2), "</b> * x + ", round(intercept, 2)))
...
x_range <- c(-0.28, 0.32)  # Adjust these values according to your needs
...
p2 <- data_plot %>% ggplot(aes(x=GPPtrend_lm, y=NEE_trend)) +
...
ggsave(file.path(dir_fig, 'flux_trend.png'), width = 8, height = 6, units = "in")
...
# NEE_trend: t = -0.69308, df = 21, p-value = 0.4959; mean difference: -0.02897602;
# GPP_trend: t = 1.7289, df = 21, p-value = 0.0985; mean difference: 0.08818296;
# ER_trend: t = 0.26294, df = 21, p-value = 0.7952; mean difference: 0.01177787; 
...
# NEE_trend: t = -2.359, df = 17, p-value = 0.03055; -0.08680022;
# GPP_trend: t = 2.4506, df = 17, p-value = 0.02538; mean difference: 0.1316831
# ER_trend: t = 0.7689, df = 17, p-value = 0.4525; mean difference: 0.03630142
...
p2 <- sub_time_series_2period_paired %>% mutate(period = factor(period, levels=c('Before', 'After'))) %>%
  ...
  geom_dotplot(binaxis = "y", stackdir = "center", alpha = 0.2) +
  geom_hline(yintercept=0, linetype = "dotted", colour = "grey") +
  labs(x='Before and after trend shift year', y=expression('Trend (year'^{-1}~')'), tag ='b', title='Long-term sites in wet-warm climate') +
...
total_es <- read.csv('data/total_effect_NEE_trend_driver.csv')
...
total_es %>% 
  ...
  mutate(var_name=factor(var_name, levels = c("Canopy height", "VPD_ngs_t", "TA_gs_t", "Lgs_t", "SW_gs_t", "ET_gs_t", "P_gs_t"))) %>%
  mutate(driver_type = case_when(var_name %in% c("P_gs_t", "ET_gs_t", "SW_gs_t", "TA_gs_t", "Lgs_t") ~ "grow-season", 
                                 var_name %in% c("VPD_ngs_t") ~ "non-grow-season",
...
p1 <- sub_time_series %>% filter(category=='dry') %>% 
  pivot_longer(cols =c('P_gs_t', 'VPD_ngs_t', 'TA_gs_t'), names_to = "driver", values_to = "driver_trend") %>% 
  mutate(driver = factor(driver, levels=c('VPD_ngs_t', 'P_gs_t', 'TA_gs_t'))) %>%
...
#  geom_jitter() +
  ...
  scale_fill_manual(values=c('#E8C999', 'blue', 'red')) +
...
p5 <- sub_time_series %>% filter(category=='cold' & !IGBP %in% c("DBF", "MF", 'ENF', 'EBF')) %>% 
  rename('ET_gs_t' = 'LE_gs_t') %>%
  pivot_longer(cols =c('P_gs_t', 'ET_gs_t'), names_to = "driver", values_to = "driver_trend") %>% 
  mutate(driver = factor(driver, levels=c('P_gs_t', 'ET_gs_t'))) %>%
  ggplot(aes(y=driver, x=driver_trend, fill=driver)) +
  geom_density_ridges(scale=0.8, alpha=0.5) +
#  geom_boxplot(outlier.shape = NA, alpha=0.1, width=0.2) +
  geom_vline(xintercept=0, linetype = "dotted", colour = "grey") +
  scale_fill_manual(values=c('blue', 'royalblue')) +
  scale_x_continuous(limits = c(-0.5, 0.5), breaks=seq(-0.4, 0.4, 0.2)) +
  labs(y='Climatic drivers of NEE trends', x='Trends of climatic drivers') +
  coord_flip() +
  theme_classic() +
