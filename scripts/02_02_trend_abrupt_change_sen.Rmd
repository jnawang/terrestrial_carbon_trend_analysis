...
sites_long <- read.csv(file.path('data', 'long_term_ec_sites.csv'))
rm_years   <- read.csv(file.path('data', 'years_to_remove.csv'))
#
...
#
a_YY_all %>% filter(site_ID %in% c("BE-Vie", "CH-Dav", "DE-Tha", "FI-Hyy", "US-Ha1")) %>% 
...
sub_time_series <- data.frame(site_ID=character(), year_start=integer(), year_end=integer(), 
                              ...
                              ER=double(), ERtrend_lm=double(), ERp=double(), NEEtrend_unit=double())   # NEEtrend_unit is not normalized. 
#
  ...
  if (site_ID %in% abrupt_change$site_ID) {
    ...
    for (iyear_ac in 1:length(years_abrupt_change)) {
      if (years_abrupt_change[iyear_ac] - year_start + 1 >= threshold_nyear - 1) {
        ...
        sub_time_series[isub_ts, "NEEtrend_unit"] <- sub_time_series[isub_ts, "NEEtrend_lm"] * sd(a_YY_sub$NEE_VUT_REF, na.rm=T)
        #
        ...
        if (sum(!is.na(a_YY_sub$GPP_DT_VUT_REF)) >= 5 & !site_ID %in% c("BR-Sa1", "US-NC2", "US-SP3", "US-WCr", "US-Ho1", "US-Ho2", "US-NC4", "US-Kon", "US-MOz", "CH-Lae", "US-Oho", "DE-RuW", "US-Uaf")) {
          ...
          # # Correct GPP and ER slope to use the same standarization with NEE, in order to compare with NEE slope on 7/5/2025
          sub_time_series[isub_ts, "GPPtrend_lm"] <- sub_time_series[isub_ts, "GPPtrend_lm"] * sd(a_YY_sub$GPP_DT_VUT_REF, na.rm=T) #/ sd(a_YY_sub$NEE_VUT_REF, na.rm=T)
          sub_time_series[isub_ts, "ERtrend_lm"] <- sub_time_series[isub_ts, "ERtrend_lm"] * sd(a_YY_sub$RECO_DT_VUT_REF, na.rm=T) #/ sd(a_YY_sub$NEE_VUT_REF, na.rm=T)
          ...
          if (sum(!is.na(a_YY_sub$GPP_NT_VUT_REF)) >= 5) {
            ...
            # Correct GPP and ER slope to use the same standarization with NEE, in order to compare with NEE slope on 7/5/2025
            sub_time_series[isub_ts, "GPPtrend_lm"] <- sub_time_series[isub_ts, "GPPtrend_lm"] * sd(a_YY_sub$GPP_NT_VUT_REF, na.rm=T) #/ sd(a_YY_sub$NEE_VUT_REF, na.rm=T)
            sub_time_series[isub_ts, "ERtrend_lm"] <- sub_time_series[isub_ts, "ERtrend_lm"] * sd(a_YY_sub$RECO_NT_VUT_REF, na.rm=T) #/ sd(a_YY_sub$NEE_VUT_REF, na.rm=T)
...
summary(lm(data=sub_time_series, ERtrend_lm ~ GPPtrend_lm + 0))  
# this is Intriguing: GPPtrend_lm = 0.025852 + 0.70053 * ERtrend_lm;
# Coefficients:
#             Estimate Std. Error t value Pr(>|t|)    
# GPPtrend_lm   0.6994     0.0616   11.35   <2e-16 ***

summary(lm(data=sub_time_series, NEEtrend_unit ~ GPPtrend_lm + 0))
#             Estimate Std. Error t value Pr(>|t|)   
# GPPtrend_lm -0.15742    0.05802  -2.713  0.00763 **    this is overall effects. 
...
p1 <- ggplot(sub_time_series, aes(x=IGBP, y=NEEtrend_unit)) +
  geom_jitter() +
  geom_boxplot()
p1
...
#############################################what if I group them into different climate-vegetation classes#####################
...
#---dry climate
data <- sub_time_series %>% filter(category=='dry') # %>% filter(GPPtrend_lm > 5) 
summary(lm(data=data, NEEtrend_unit ~ GPPtrend_lm + 0))
# GPPtrend_lm  -0.1417     0.1608  -0.881    0.391
plot(data$GPPtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, NEEtrend_lm ~ ERtrend_lm + 0))
# ERtrend_lm -4.019e-05  1.399e-03  -0.029    0.977
plot(data$ERtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, ERtrend_lm ~ GPPtrend_lm + 0))
# GPPtrend_lm   0.9357     0.1575   5.941 2.07e-05 ***  # ER trend is quite similar to GPP trend.  
plot(data$GPPtrend_lm, data$ERtrend_lm)

#---wet warm climate
data <- sub_time_series %>% filter(category=='wet')   # %>% filter(GPPtrend_lm > -20) 
summary(lm(data=data, NEEtrend_unit ~ GPPtrend_lm + 0))
#             Estimate Std. Error t value Pr(>|t|)   
# GPPtrend_lm  -0.3349     0.1298  -2.579   0.0209 *
plot(data$GPPtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, NEEtrend_lm ~ ERtrend_lm + 0))
# ERtrend_lm -0.001356   0.001288  -1.052    0.309
plot(data$ERtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, ERtrend_lm ~ GPPtrend_lm + 0))
# GPPtrend_lm   0.6742     0.1523   4.426 0.000491 ***
plot(data$GPPtrend_lm, data$ERtrend_lm)

#---wet-cold ENF
data <- sub_time_series %>% filter(category=='cold') %>% filter(IGBP %in% c('ENF', 'EBF')) # %>% filter(GPPtrend_lm > -20) 
summary(lm(data=data, NEEtrend_unit ~ GPPtrend_lm + 0))
# GPPtrend_lm  -0.3058     0.1277  -2.394   0.0217 *
plot(data$GPPtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, NEEtrend_unit ~ ERtrend_lm + 0))
# ERtrend_lm -0.009692   0.150657  -0.064    0.949
plot(data$ERtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, ERtrend_lm ~ GPPtrend_lm + 0))
# GPPtrend_lm   0.4240     0.1305   3.248  0.00243 **  # this is much smaller slope
plot(data$GPPtrend_lm, data$ERtrend_lm)

#---wet-cold DBF
data <- sub_time_series %>% filter(category=='cold') %>% filter(IGBP %in% c('MF', 'DBF')) # %>% filter(abs(GPPtrend_lm) < 0.3) 
summary(lm(data=data, NEEtrend_unit ~ GPPtrend_lm + 0))
# GPPtrend_lm -0.02341    0.11768  -0.199    0.844
plot(data$GPPtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, NEEtrend_unit ~ ERtrend_lm + 0))
# ERtrend_lm   0.1694     0.1017   1.666    0.108   # more correlated with ER.
plot(data$ERtrend_lm, data$NEEtrend_unit)

# for these ecosystems more GPP increase are used. 
summary(lm(data=data, ERtrend_lm ~ GPPtrend_lm + 0))
# GPPtrend_lm   0.8926     0.1262   7.071 1.65e-07 ***
plot(data$GPPtrend_lm, data$ERtrend_lm)

#---wet-cold non-forests
data <- sub_time_series %>% filter(category=='cold') %>% filter(!IGBP %in% c('MF', 'DBF', 'ENF', 'EBF'))  %>% filter(GPPtrend_lm < 100) 
summary(lm(data=data, NEEtrend_unit ~ GPPtrend_lm + 0))
# GPPtrend_lm  -0.2706     0.1669  -1.621    0.119
plot(data$GPPtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, NEEtrend_unit ~ ERtrend_lm + 0))
# ERtrend_lm   0.3354     0.2003   1.674    0.108
plot(data$ERtrend_lm, data$NEEtrend_unit)

summary(lm(data=data, ERtrend_lm ~ GPPtrend_lm + 0))
# GPPtrend_lm   0.3374     0.1582   2.132   0.0439 *  # this is also small. 
plot(data$GPPtrend_lm, data$ERtrend_lm)
